...
function! ElelineError() abort
	if exists('g:loaded_ale') && get(g:, 'ale_enabled', 1)
		...
		return s:ale_counts[0] == 0 ? '' : '╬ô├ç├│'.s:ale_counts[0].' '
	elseif exists('b:clap_diagnostics')
		let error_count = b:clap_diagnostics.error
		return error_count == 0 ? '' : '╬ô├ç├│'.error_count.' '
	endif
	return ''
endfunction
...
function! ElelineWarning() abort
	if exists('g:loaded_ale') && get(g:, 'ale_enabled', 1)
		...
		return s:ale_counts[1] == 0 ? '' : '╬ô├ç├│'.s:ale_counts[1].' '
	elseif exists('b:clap_diagnostics')
		let warn_count = b:clap_diagnostics.warn
		return warn_count == 0 ? '' : '╬ô├ç├│'.warn_count.' '
	endif
	return ''
endfunction
...
function! ElelineLspStatus() abort
    if !has('nvim-0.5') && !exists('*lsp#get_server_status')
        return ''
    endif

    let l:counts = {'information': 0, 'hint': 0, 'warning': 0, 'error': 0}
        ...
        for _, d in pairs(diagnostics) do
            for _, diag in ipairs(d) do
                if diag.severity == vim.lsp.protocol.DiagnosticSeverity.Error then
                    vim.g.eleline_error_count = vim.g.eleline_error_count + 1
                elseif diag.severity == vim.lsp.protocol.DiagnosticSeverity.Warning then
                    vim.g.eleline_warning_count = vim.g.eleline_warning_count + 1
                end
            end
        end
EOF
        let l:counts.error = g:eleline_error_count
        let l:counts.warning = g:eleline_warning_count
    ...
    let l:result = ''
    if l:counts.error > 0
        let l:result .= ' ' . 'Γò│' . l:counts.error . ' '
    endif
    if l:counts.warning > 0
        let l:result .= ' ' . 'ΓÜá' . l:counts.warning . ' '
    endif

    return l:result
endfunction

function! ElelineFormatStatus() abort
    let l:mixed_indent = search('\v(^\t+ +)|(^ +\t+)', 'nw') != 0
    let l:trailing_space = search('\s$', 'nw') != 0
...
function! s:StatusLine() abort
	...
	let l:common = l:curfname
				\ .l:branch.l:status
				" \ .l:error.l:warning
				\ .l:tags.l:lcn.l:coc.l:func
	...
	" Total number of words in the buffer
	let l:words = '%#Eleline9# '. '%{wordcount().words}'
	let l:lsp_stats = '%#Eleline9#%{ElelineLspStatus()}%*'
	let l:format = '%#Eleline9#%{ElelineFormatStatus()}%*'
	return l:prefix.l:tot.'%<'.l:fsize.l:common
				\ .'%='
				\ .l:lsp_stats
				\ .l:format
				\ .l:m_r_f.l:pos.l:enc.l:ff.l:pct.l:words
endfunction
...
function! s:hi_statusline() abort
	...
	call s:hi('ElelineLspStatus'  	, [171 , s:bg+4] , [171 , ''])
	call s:hi('ElelineFormatStatus' , [171 , s:bg+4] , [171 , ''])
